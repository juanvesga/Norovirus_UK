...
outfile <- file.path(root, "output", "polymod_short.qs2")
outfile2 <- file.path(root, "output", "polymod_unweighted_short.qs2")
...
polymod$contact_matrix<- symm_mat(contact$matrix)

polymod$contact_matrix_holi<- symm_mat(contact_holi$matrix)

# Uncorrected matrices

M<-list(
  m     =symm_mat(contact$matrix),
  m_holi=symm_mat(contact_holi$matrix)
)
