get_R0<-function(parms,time_vec,get_addresses,strain){
  ...
  beta_gii4  <- exp(parms$log_beta_3)   
  beta_gi3   <- beta_gii4 * parms$factor_beta_1 
  beta_gi    <- beta_gii4 * parms$factor_beta_2 
  beta_gii   <- beta_gii4 * parms$factor_beta_4  
  aduRR      <- exp(parms$log_aduRR)
  crossp_GI  <- parms$crossp_GI
  crossp_GII <- parms$crossp_GI
  ...
  gps<-list(
    states=c("E","I","A"),
    age   =c("a1","a2","a3","a4","a5","a6","a7","a15","a65","a75"))
  ...
  age_infectivity[8:nrow(C)]<-aduRR
  ...
  seasonality <-(1 + season_amp* cos(2 * pi * (tt - (season_lag-20)) /365))
