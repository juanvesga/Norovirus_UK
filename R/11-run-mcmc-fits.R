...
tmp     <- readRDS(infile_chains) 
trace <- coda::as.mcmc.list(tmp)
...
#get last sample
params<- (longchain[sample(nrow(longchain),size=nsamps,replace=FALSE),])
...
foreach_fun<-function(params,
  ...
  foreach(i = 1:nsamps) %dopar% {
    ...
    # 
    list(out,init_state0,pars,index)
...
parameters <- as.data.frame(do.call(rbind,lapply(results,function(x){x[[3]]})))


id<-results[[4]]


index <- as.data.frame(do.call(rbind,lapply(results,function(x){x[[4]]})))



index1 <- lapply(index, function(x) x[[1]])
