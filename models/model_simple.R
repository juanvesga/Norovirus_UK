...
noro_model <- odin({
  ...
  p_RE[, ,]  <- 1 - exp(- lambda_cross[i,j,k] * dt) # S to I age/vacc dependent
...
# dim(rel_foi_cross) <- c(N_age, N_strain, N_strain)
 ...
 n_S_E[,] <- if (j==1) Binomial(S[i] - n_ageoS[i] - n_muS[i], p_SE[i,j]) else
  ...
  #n_S_E_tot[]         <- sum(n_S_E_tot_strain[i,]) 
  
  #browser(phase = "update", when = time ==21)
  ...
  # Transitions I to A
  n_I_A[,] <- Binomial( I[i,j] - n_ageoI[i,j] - n_muI[i,j], 
                       if (i <= 5) p_IA_5 else p_IA_5p)
...
#  n_R_progress[, , ] <- if (k == 1) Binomial(n_R_tot[i, j], rel_foi_cross[i, j, 1]) else
...
#       n_R_tot[i, j] - sum(n_R_progress[i, j, 1:3]))
  ...
  # n_R_rel1[,2:3] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #   Binomial(n_R_tot[i,1] - sum(n_R_rel1[i, 1:(j-1)]), 
  #            min(rel_foi_strain[i,j]/sum(rel_foi_strain[i, j:N_strain]), as.numeric(1)))
  
  # n_R_rel1[, 4] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #   n_R_tot[i, 1] - sum(n_R_rel1[i, 1:3])
  ...
  # n_R_rel2[,2:3] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #   Binomial(n_R_tot[i,2] - sum(n_R_rel2[i, 1:(j-1)]), 
  #            min(rel_foi_strain[i,j]/sum(rel_foi_strain[i, j:N_strain]), as.numeric(1)))

  #  n_R_rel2[, 4] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0 else 
  #   n_R_tot[i, 2] - sum(n_R_rel2[i, 1:3])
  
  

  # R[3]
  #  n_R_rel3[,1] <- Binomial(n_R_tot[i,3], rel_foi_strain[i,1])
  #  n_R_rel3[,2:3] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #    Binomial(n_R_tot[i,3] - sum(n_R_rel3[i, 1:(j-1)]), 
  #             min(rel_foi_strain[i,j]/sum(rel_foi_strain[i, j:N_strain]), as.numeric(1)))

  # n_R_rel3[, 4] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #   n_R_tot[i, 3] - sum(n_R_rel3[i, 1:3])
  

  # R[4]
  # n_R_rel4[,1] <- Binomial(n_R_tot[i,4], rel_foi_strain[i,1])
  # n_R_rel4[,2:3] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #   Binomial(n_R_tot[i,4] - sum(n_R_rel4[i, 1:(j-1)]), 
  #            min(rel_foi_strain[i,j]/sum(rel_foi_strain[i, j:N_strain]), as.numeric(1)))
  
  # n_R_rel4[, 4] <- if (sum(rel_foi_strain[i, j:N_strain]) == 0) 0.0 else 
  #   n_R_tot[i, 4] - sum(n_R_rel4[i, 1:3])

  
  
  n_R_E[,1]  <- n_R_progress[i,2,1]  + n_R_progress[i,3,1] + n_R_progress[i,4,1] 
  n_R_E[,2]  <- n_R_progress[i,1,2]  + n_R_progress[i,3,2] + n_R_progress[i,4,2]
  n_R_E[,3]  <- n_R_progress[i,4,3]  + n_R_progress[i,1,3] + n_R_progress[i,2,3]  
  n_R_E[,4]  <- n_R_progress[i,3,4]  + n_R_progress[i,1,4] + n_R_progress[i,2,4]
...
#  initial(new_hosp_elder_yr, zero_every = 365) <- 0
  ...
  # Parameters --------------------------------------------------------------
  #browser(phase = "update", when = time ==0)
