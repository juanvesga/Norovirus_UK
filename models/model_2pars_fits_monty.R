...
noro_model <- odin({
 ...
 # browser(phase = "update", when = time ==21360)
...
# )
  ...
  # )
  # 
  # rel_foi_cross[,,]<- if (sum(lambda_cross[i,j,]) == 0) 0 else(
  #   if (k < N_strain)
  #     max(
  #       1-(1-(min(lambda_cross[i, j,k] / sum(lambda_cross[i,j,]),as.numeric(1)))),
  #       as.numeric(0)
  #     ) else
  #       max(min(1-sum(rel_foi_cross[i,j, 1:3]),as.numeric(1)),as.numeric(0))
  # )
